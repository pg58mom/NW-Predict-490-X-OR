...
# Initialize the bias weight variables with 4 selected values
    biasWeight00=-10
    biasWeight01=30
    biasWeight10=-30
    biasWeight11=30

# Store the four bias weights in a 2-D array            
    biasWeightArray=np.array([[biasWeight00,biasWeight01],[biasWeight10,biasWeight11]])
...
# Assign the hidden-to-output weights to specific variables
    ...
    biasHidden0 = biasHiddenWeightArray[0,0]
    biasHidden1 = biasHiddenWeightArray[0,1]
    biasOutput0 = biasOutputWeightArray[1,0]
    biasOutput1 = biasOutputWeightArray[1,1]
...
#    the initial values that we've just obtained. 
    ...
    print '       b(hidden,0) = %.4f                           b(output,0) = %.4f' % (biasHiddenWeightArray[0,0],
    biasOutputWeightArray[1,0] )                  
    print '       b(hidden,1) = %.4f                           b(output,1) = %.4f' % (biasHiddenWeightArray[0,1],
    biasOutputWeightArray[1,1] )  
  
# Establish some parameters just before we start training
    epsilon = 0 # epsilon determines when we are done training; 
